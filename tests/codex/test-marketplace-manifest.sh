...
assert_equal(marketplace.get("name"), "localSuperpowers-dev", "marketplace name")
assert_equal(
    marketplace.get("interface", {}).get("displayName"),
    "localSuperpowers Dev",
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "localSuperpowers"]
assert_equal(len(matching_plugins), 1, "localSuperpowers plugin entry count")
